...
function initThree() {
  ...
  scene.add(light1);
  const light2 = new THREE.DirectionalLight(0xff0000, 0.6);
  ...
  leftSleeve.position.set(-1.1, 0, 0);
  leftSleeve.rotation.set(0, 0, Math.PI / 2);
  scene.add(leftSleeve);
  
  const rightSleeve = leftSleeve.clone();
  rightSleeve.position.x = 1.1;
...
function addDragListeners() {
  ...
  window.addEventListener('mousemove', e => {
    ...
    rotationY += (e.clientX - previousX) * 1.8;
    previousX = e.clientX;
  });
  window.addEventListener('touchmove', e => {
    if (!isDragging) return;
    rotationY += (e.touches[0].clientX - previousX) * 1.8;
...
function changeColor(color) {
  currentColor = color;
  if (shirt) shirt.material.color.set(color);
}
...
function loadAngryDuckDesign() {
  ...
  canvasTexture.needsUpdate = true;
  alert('🐤 Angry Duck Design geladen – dreh das Shirt!');
}
...
function uploadImage(e) {
  ...
  reader.onload = function(ev) {
    const img = new Image();
    img.onload = function() {
      designCtx.drawImage(img, 200, 200, 624, 624);
      canvasTexture.needsUpdate = true;
    };
...
function addToCart() {
  ...
  console.log('%c✅ Design gespeichert – bereit für Printful/Printify', 'color:#ff0000;font-size:16px');
  const toast = document.createElement('div');
  toast.style.cssText = 'position:fixed;bottom:24px;right:24px;background:#ff0000;color:white;padding:20px 32px;border-radius:9999px;font-weight:700;box-shadow:0 25px 50px -12px rgb(0 0 0 / 0.4);';
...
function toggleCart() {
  alert('🛒 Warenkorb (Phase 3 wird richtig mit Printful)');
}
...
window.onload = () => {
  initThree();
  addDragListeners();
  console.log('%c🚀 Phase 2 – ECHTES Three.js 3D gestartet! Besser als Zakeke.', 'color:#ff0000;font-size:18px;font-weight:bold');
};
